...
import tictim.hearthstones.client.gui.TavernButton.TavernProperty;
...
public class HearthstoneScreen extends TavernMemoryScreen{
	...
	public void updateData(OpenHearthstoneScreenMsg packet){
		this.playerMemory = packet.getPlayerMemory();
		this.globalMemory = packet.getGlobalMemory();
	...
	@Override protected List<TavernButton> createTavernButtons(){
		...
		for(TavernRecord t : playerMemory.taverns().values()){
			TavernButton b = new TavernButton(0, this, t);
			b.canSelect = true;
			b.canDelete = true;
			addProperties(b);
			buttons.add(b);
		...
		buttons.sort((b1, b2) -> {
			...
			if(d1==d2&&dim==d1){
				double s1 = mc.player.getDistanceSq(o1.blockPos().getX()+0.5, o1.blockPos().getY()+0.5, o1.blockPos().getZ()+0.5);
				double s2 = mc.player.getDistanceSq(o2.blockPos().getX()+0.5, o2.blockPos().getY()+0.5, o2.blockPos().getZ()+0.5);
	...
	private void addProperties(TavernButton button){
		...
		if(hearthingGem&&HearthingGemHearthstone.isTooFar(mc.player, button.tavern.pos()))
	...
	@Nullable @Override protected String getEmptyScreenMessage(){
		return I18n.format("info.hearthstones.screen.empty");
	}
}